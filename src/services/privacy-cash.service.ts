...
export class PrivacyCashService {
  ...
  async checkAvailability(): Promise<{ available: boolean; error?: string }> {
    ...
    try {
      ...
      }) as unknown as PrivacyCashClient;
  ...
  /**
   * Get anonymity set information for a given token
   */
  static getAnonymitySetInfo(tokenSymbol: string): {
    ...
    const pools: Record<string, { minAnonymitySet: number; estimatedPoolValue: string }> = {
      SOL: { minAnonymitySet: 100, estimatedPoolValue: '500-2000 SOL' },
      USDC: { minAnonymitySet: 150, estimatedPoolValue: '100K-500K USDC' },
      USDT: { minAnonymitySet: 150, estimatedPoolValue: '100K-500K USDT' },
    };
